@extends('layouts.app')
		      ...
		      <div id="content">
		        ...
		        <!-- Begin Page Content -->
			        <div class="container-fluid">
			          ...
			          <!-- Page Heading -->
			          <h1 class="h3 mb-2 text-gray-800">Calificaciones</h1>
			          ...
			          <div class="card shadow mb-4" id="cuadro1">
			            <div class="card-header py-3">
			              <h6 class="m-0 font-weight-bold text-primary">Gestion de Calificaciones</h6>
			          ...
			          @include('tasks.advisers.store')
					  @include('tasks.advisers.edit')
	...
	@section('CustomJs')
		...
		<script>
			...
			$("#date_init, #date_finish, #type-filter, #linea-negocio-filter").change(function (e) {
				list();
			});
			...
			function update(){
				enviarFormularioPut("#form-update", 'api/califications/advisers', '#cuadro4', false, "#avatar-edit");
			}
			...
			function list(cuadro) {
                ...
                const line = $("#linea-negocio-filter").val()
				...
				var table=$("#table").DataTable({
					...
					"ajax":{
						"method":"GET",
						 "url":''+url+'/api/califications/advisers',
						 "data": {
							"rol"     : name_rol,
							"id_user" : id_user,
							"token"   : tokens,
			...
			*/
			...
			function edit(tbody, table){
				$(tbody).on("click", "span.editar", function(){
					...
					var url_imagen = '/img/califications/'
					var url        = document.getElementById('ruta').value;

					if((data.evidence != "" ) &&  (data.evidence != null)){
						var ext = data.evidence.split('.');
						if (ext[1] == "pdf") {
							img = '<embed class="kv-preview-data file-preview-pdf" src="'+url+url_imagen+data.evidence+'" type="application/pdf" style="width:213px;height:160px;" internalinstanceid="174">'
						}else{
							img = '<img src="'+url+url_imagen+data.evidence+'" class="file-preview-image kv-preview-data">'
						}
					...
					$("#evidence_edit").fileinput('destroy');
					$("#evidence_edit").fileinput({
						theme: "fas",
						overwriteInitial: true,
						maxFileSize: 1500,
						showClose: false,
